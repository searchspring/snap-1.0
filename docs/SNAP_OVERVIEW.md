...
Each controller contains a `controller.search()` method that is used to trigger a search to Athos API with parameters derived from the `controller.urlManager` state. A urlManager contains the state of the url (query and hash parameters), or in the case of Autocomplete, Chat, Recommendations, and Finder, is detached from the url and contains it's own state.

Data that is returned from the API is then stored in each controller's `controller.store`. The data schema is unique to each controller type (Search, Autocomplete, Chat, Recommendations, Finder) 
