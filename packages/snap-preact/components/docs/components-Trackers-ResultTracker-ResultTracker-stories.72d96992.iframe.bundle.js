				...
				const R = { ResultTracker: () => (0, _.AH)({}) },
					y = (0, u.PA)((f) => {
						const g = (0, s.u)(),
							...
							{ children: P, result: E, track: D, controller: p, className: O, internalClassName: k, disableStyles: C, style: l } = m,
							i = { ...v, ...D },
							{ ref: h, inViewport: T } = (0, c.Q)();
						T && i.impression && p?.track.product.impression(E);
						const d = {};
						return (
							C ? l && (d.css = [l]) : (d.css = [R.ResultTracker(), l]),
							(0, o.Y)('div', {
			...
			'./components/src/utilities/snapify.ts'(M, a, t) {
				...
				class p {
					...
					static autocomplete(i) {
						const h = i.id;
						if (E[h]) return E[h];
						const T = (E[h] = C({ client: D, controller: i }));
						return (
							T.on('afterStore', async ({ controller: d }, I) => {
								d.log.debug('controller', d), d.log.debug('store', d.store.toJSON()), await I();
							}),
...
//# sourceMappingURL=components-Trackers-ResultTracker-ResultTracker-stories.72d96992.iframe.bundle.js.map
