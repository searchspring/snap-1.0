	...
	[1555],
	{
		...
		'./components/src/components/Organisms/Facet/Facet.stories.tsx': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			...
			const Facet_stories = {
				...
				snapInstance = snapify.p.search({ id: 'Facet', globals: { siteId: 'atkzs2' } }),
				ObservableListFacet = (0, es.PA)(({ args, controller }) =>
					(0, emotion_react_jsx_runtime_browser_esm.Y)(Facet.s, {
						...args,
						facet: controller?.store?.facets.filter((facet) => facet.display === types.Q.LIST).shift(),
					})
				),
			...
			const ObservableSliderFacet = (0, es.PA)(({ args, controller }) => {
					const facet = controller?.store?.facets.filter((facet) => facet.display === types.Q.SLIDER).shift();
					return facet
			...
			const ObservablePaletteFacet = (0, es.PA)(({ args, controller }) => {
					const facet = controller?.store?.facets.filter((facet) => facet.display === types.Q.PALETTE).shift();
					return facet
						? (0, emotion_react_jsx_runtime_browser_esm.Y)(Facet.s, { ...args, facet })
						: (0, emotion_react_jsx_runtime_browser_esm.Y)('div', {});
				}),
			...
			const ObservableGridFacet = (0, es.PA)(({ args, controller }) => {
					const facet = controller?.store?.facets.filter((facet) => 'collection_handle' === facet.field).pop();
					return facet
						? (0, emotion_react_jsx_runtime_browser_esm.Y)(Facet.s, { ...args, facet })
						: (0, emotion_react_jsx_runtime_browser_esm.Y)('div', {});
				}),
			...
			const ObservableHierarchyFacet = (0, es.PA)(({ args, controller }) => {
					const facet = controller?.store?.facets.filter((facet) => facet.display === types.Q.HIERARCHY).shift();
					return facet
						? (0, emotion_react_jsx_runtime_browser_esm.Y)(Facet.s, { ...args, facet })
						: (0, emotion_react_jsx_runtime_browser_esm.Y)('div', {});
				}),
