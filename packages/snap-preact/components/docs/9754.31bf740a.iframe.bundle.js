	...
	[9754],
	{
		'../snap-controller/dist/esm/Autocomplete/AutocompleteController.js': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			...
			var deepmerge__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__('../../node_modules/deepmerge/dist/cjs.js'),
				...
				_searchspring_snap_store_mobx__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__('../snap-store-mobx/dist/esm/Storage/StorageStore.js'),
				...
				_utils_getParams__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__('../snap-controller/dist/esm/utils/getParams.js'),
			...
			class AutocompleteController extends _Abstract_AbstractController__WEBPACK_IMPORTED_MODULE_2__.r {
				constructor(config, { client, store, urlManager, eventManager, profiler, logger, tracker }, context) {
					super(config, { client, store, urlManager, eventManager, profiler, logger, tracker }, context),
						(this.type = _types__WEBPACK_IMPORTED_MODULE_3__.k.autocomplete),
						(this.events = { product: {} }),
						(this.track = {
							product: {
								...
								addToCart: (result) => {
									const data = (function getAutocompleteAddtocartSchemaData({ params, store, results }) {
									...
									})({ params: this.params, store: this.store, results: [result] });
							...
							redirect: (redirectURL) => {
								const data = (function getAutocompleteRedirectSchemaData({ redirectURL }) {
						...
						(this.handlers = {
							input: {
								enterKey: async (e) => {
									if (13 == e.keyCode) {
										...
										if ((e.preventDefault(), this.config.globals?.search?.query?.spellCorrection)) {
											for (; this.store.loading; ) await timeout(200);
										...
										(actionUrl = actionUrl?.set('query', input.value)), await timeout(201);
										try {
								...
								input: (e) => {
									...
									if (((this.store.state.input = value), (this.store.merchandising.redirect = ''), this.config?.settings?.syncInputs)) {
							...
							document: {
								click: (e) => {
									...
									Array.from(inputs).includes(e.target) || ('FORM' == e.target?.nodeName && e.target.querySelectorAll(this.config.selector).length)
										? e.stopPropagation()
										: this.setFocused();
								},
							},
						...
						(this.searchTrending = async (options) => {
							...
							if (storedTerms && !options?.limit) trending = JSON.parse(storedTerms);
							else {
								const trendingParams = { limit: options?.limit || this.config.settings?.trending?.limit || 5 },
						...
						(this.search = async () => {
							try {
								...
								if (!params?.search?.query?.string) return;
								...
								if ((searchProfile.stop(), this.log.profile(searchProfile), search?.search?.query === this.lastSearchQuery)) {
									const impressedResultIds = Object.keys(this.events.product).filter((resultId) => this.events.product[resultId]?.impression);
									this.events = { product: impressedResultIds.reduce((acc, resultId) => ((acc[resultId] = { impression: !0 }), acc), {}) };
								} else (this.events = { product: {} }), (this.lastSearchQuery = search?.search?.query);
						...
						this.config.settings?.trending?.limit &&
							void 0 === this.config.settings?.trending?.enabled &&
						...
						this.config.settings?.history?.limit &&
							void 0 === this.config.settings?.history?.enabled &&
						...
						(this.storage = new _searchspring_snap_store_mobx__WEBPACK_IMPORTED_MODULE_6__.t({
							type: 'session',
						...
						this.eventManager.on('afterSearch', async (ac, next) => {
							if ((await next(), ac.response.search.autocomplete?.query != ac.controller.urlManager.state.query)) return !1;
						}),
						this.eventManager.on('beforeSubmit', async (ac, next) => {
							await next();
							if (ac.controller.store.loading) return;
							const inputState = ac.controller.store.state.input,
								redirectURL = ac.controller.store.merchandising?.redirect;
							if (this.config?.settings?.redirects?.merchandising && inputState && redirectURL)
								return this.track.redirect(redirectURL), (window.location.href = redirectURL), !1;
							if (this.config?.settings?.redirects?.singleResult) {
								const { results } = ac.controller.store,
									filteredResults = results.filter((result) => 'product' == result.type),
									singleResultUrl = 1 === filteredResults.length && filteredResults[0].mappings.core?.url;
				...
				get params() {
					const urlState = this.urlManager.state,
						params = deepmerge__WEBPACK_IMPORTED_MODULE_0___default()(
							{ ...(0, _utils_getParams__WEBPACK_IMPORTED_MODULE_7__.j)(urlState) },
							this.config.globals || {}
						),
						{ userId, sessionId, pageLoadId, shopperId } = this.tracker.getContext();
					if (
			...
			function getAutocompleteSchemaData({ params, store, results }) {
				...
				return {
					...
					correctedQuery: store.search?.originalQuery?.string ? store.search?.query?.string : void 0,
			...
			class AutocompleteStore extends AbstractStore.K {
				...
				update(data) {
					...
					(this.meta = new MetaStore.l({ data: { meta } })),
						...
						(this.filters = new SearchFilterStore.Al({ config: this.config, services: this.services, data: { search, meta: this.meta.data } })),
