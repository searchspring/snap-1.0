			...
			'./components/src/utilities/snapify.ts'(E, d, t) {
				...
				class g {
					static recommendation(i) {
						...
						return (
							f.on('afterStore', async ({ controller: p }, w) => {
					...
					static autocomplete(i) {
						const m = i.id;
						if (l[m]) return l[m];
						const f = (l[m] = M({ client: b, controller: i }));
						return (
							f.on('afterStore', async ({ controller: p }, w) => {
								p.log.debug('controller', p), p.log.debug('store', p.store.toJSON()), await w();
							}),
...
//# sourceMappingURL=components-Molecules-Result-Result-stories.62553fca.iframe.bundle.js.map
