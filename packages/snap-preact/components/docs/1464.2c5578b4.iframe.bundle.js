						...
						this.eventManager.on('afterStore', async (search, next) => {
							...
							if (controller.store.loaded && !controller.store.error) {
								const products = controller.store.results.filter((result) => 'product' === result.type),
									...
									data = getAutocompleteSchemaData({ params: search.request, store: this.store, results });
								search.response._cached || this.tracker.events.autocomplete.render({ data, siteId: this.config.globals?.siteId }),
