		...
		'../../node_modules/react-fast-compare/index.js'(exports, module) {
			...
			function equal(a, b) {
				...
				if (a && b && typeof a == 'object' && typeof b == 'object') {
					...
					for (i = length; i-- !== 0; )
						if (!((keys[i] === '_owner' || keys[i] === '__v' || keys[i] === '__o') && a.$$typeof) && !equal(a[keys[i]], b[keys[i]])) return !1;
					return !0;
				}
				return a !== a && b !== b;
			}
...
var fromEntries = function (entries) {
	...
	useIsomorphicLayoutEffect = typeof window < 'u' && window.document && window.document.createElement ? React3.useLayoutEffect : React3.useEffect,
	top = 'top',
	bottom = 'bottom',
	right = 'right',
	left = 'left',
...
function computeStyles(_ref5) {
	...
	state.modifiersData.popperOffsets != null &&
		(state.styles.popper = Object.assign(
			...
			state.styles.popper,
			mapToStyles(
				Object.assign({}, commonStyles, { offsets: state.modifiersData.popperOffsets, position: state.options.strategy, adaptive, roundOffsets })
			)
		)),
		state.modifiersData.arrow != null &&
			(state.styles.arrow = Object.assign(
				{},
				state.styles.arrow,
				mapToStyles(Object.assign({}, commonStyles, { offsets: state.modifiersData.arrow, position: 'absolute', adaptive: !1, roundOffsets }))
			)),
...
function getDocumentRect(element) {
	...
	return (
		getComputedStyle(body || html).direction === 'rtl' && (x += max(html.clientWidth, body ? body.clientWidth : 0) - width), { width, height, x, y }
	);
}
				...
				placements2 = [preferredPlacement].concat(fallbackPlacements).reduce(function (acc, placement2) {
					return acc.concat(
						getBasePlacement(placement2) === auto
							? computeAutoPlacement(state, { placement: placement2, boundary, rootBoundary, padding, flipVariations, allowedAutoPlacements })
							: placement2
					);
				}, []),
	...
	return (
		(isOffsetParentAnElement || (!isOffsetParentAnElement && !isFixed)) &&
			...
			isHTMLElement(offsetParent)
				? ((offsets = getBoundingClientRect(offsetParent, !0)), (offsets.x += offsetParent.clientLeft), (offsets.y += offsetParent.clientTop))
			...
			if (getLatest().finalConfig.closeOnOutsideClick) {
				var handleClickOutside = function (event) {
					var _event$composedPath,
						...
						target =
							(event.composedPath == null || (_event$composedPath = event.composedPath()) == null ? void 0 : _event$composedPath[0]) || event.target;
...
var match = (0, import_memoizerific.default)(1e3)((requests, actual, value, fallback = 0) => (actual.split('-')[0] === requests ? value : fallback)),
	...
	Tooltip = import_react.default.forwardRef(({ placement, hasChrome, children, arrowProps, tooltipRef, color, withArrows, ...props }, ref) =>
