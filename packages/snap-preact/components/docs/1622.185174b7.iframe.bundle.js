		...
		'./src/Templates/Stores/library/languages/fr.ts': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			...
			const fr = {
				...
				pagination: {
					...
					last: { attributes: { 'aria-label': (data) => `aller à la dernière page ${data?.paginationStore?.last?.number}` } },
					page: { attributes: { 'aria-label': (data) => `aller à la page ${data?.page?.number}` } },
				},
				loadMore: {
					loadMoreButton: { attributes: { 'aria-label': 'Charger plus' } },
					progressText: { value: (data) => `Vous avez consulté ${data?.paginationStore?.end} de ${data?.paginationStore?.totalResults} produits` },
				},
