			...
			const defaultStyles = ({
				...
				FacetSlider = (0, es.PA)((properties) => {
					const globalTheme = (0, emotion_element_5486c51c_browser_esm.u)(),
						...
						{ showTicks, facet, stickyHandleLabel, separateHandles, onChange, onDrag, className, internalClassName } = props;
					let { tickSize } = props;
					tickSize =
					...
					const [values, setValues] = (0, hooks_module.J0)([facet.active?.low, facet.active?.high]),
						...
						ensureValueSeparation = (val) => {
							if (!separateHandles || !facet.step) return val;
							const [minVal, maxVal] = val,
								rangeMin = facet.range?.low,
								rangeMax = facet.range?.high,
								step = facet.step;
					...
					const { getTrackProps, ticks, segments, handles } = useRanger({
							...
							onDrag: (val) => {
								const adjustedVal = ensureValueSeparation(val);
								setActive(adjustedVal), onDrag && onDrag(adjustedVal);
							},
							min: facet.range?.low,
