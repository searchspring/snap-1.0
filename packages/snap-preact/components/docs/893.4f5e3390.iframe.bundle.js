			...
			const es = {
				...
				'terms.history': {
					...
					term: {
						...
						attributes: { 'aria-label': (data) => `artículo ${data.index + 1} de ${data.numberOfTerms}, ${data.term.value}` },
					},
				},
				'terms.suggestions': {
				...
				searchInput: {
					...
					closeSearchButton: { attributes: { 'aria-label': 'Cerrar búsqueda' } },
					clearSearchButton: { attributes: { 'aria-label': 'Borrar búsqueda' } },
					submitSearchButton: { attributes: { 'aria-label': 'Enviar búsqueda' } },
				},
				recommendationBundle: {
					seedText: { value: 'Este producto' },
					ctaButtonText: { value: 'Agregar todo al carrito' },
					ctaButtonSuccessText: { value: '¡Paquete agregado!' },
					ctaSubtotalTitle: { value: (data) => `Subtotal para ${data.cartStore.count} artículos` },
				},
				mobileSidebar: {
				...
				searchHeader: {
					titleText: {
						value: (data) =>
							`Mostrando resultado${1 == data?.pagination?.totalResults ? '' : 's'} \n                ${
					...
					expandedSearchText: {
						value: (data) =>
							`No pudimos encontrar una coincidencia exacta para "<span className="ss__search-header__results-query">${data?.search?.query?.string}</span>", pero aquí hay algo similar:`,
					},
				},
				noResults: {
