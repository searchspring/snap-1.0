	...
	[8792],
	{
		...
		'../snap-client/dist/esm/Client/Client.js': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			...
			(transformSearchResponse.pagination = (response) => {
				...
				(transformSearchResponse.result = (rawResult, idx) => {
		...
		'../snap-toolbox/dist/esm/featureFlags/featureFlags.js': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			'use strict';
			__webpack_require__.d(__webpack_exports__, { o: () => featureFlags });
				...
				constructor(globals, config) {
					if (
						(((config = deepmerge__WEBPACK_IMPORTED_MODULE_0___default()(
							defaultConfig,
							config || {}
						)).initiator = `searchspring/${config.framework}/${_searchspring_snap_toolbox__WEBPACK_IMPORTED_MODULE_3__.r}`),
			...
			function configureSnapFeatures(config) {
				!(function configureIntegratedSpellCorrection(config) {
					...
					(function configureTracking(config) {
						const webPixel = window.sessionStorage?.getItem('ssWebPixel');
						if (webPixel)
							try {
			...
			class Snap {
				constructor(config, services) {
					...
					try {
						...
						const trackerConfig = cjs_default()({ framework: 'snap/preact', mode: this.mode }, this.config.tracker?.config || {});
						if (
					...
					const urlParams = url(window.location.href),
						...
						snapConfig = (function createSnapConfig(templateConfig, templatesStore) {
							const snapConfig = {
								...
								client: { globals: { siteId: templateConfig.config?.siteId }, config: { initiator: `snap/templates/${version.r}` } },
