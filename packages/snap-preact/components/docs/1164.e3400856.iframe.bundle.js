	...
	[1164],
	{
		'./src/Instantiators/RecommendationInstantiator.tsx': (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
			...
			class RecommendationInstantiator {
				constructor(config, services, context) {
					...
					this.config.mode &&
						...
						(this.tracker = services?.tracker || new _searchspring_snap_tracker__WEBPACK_IMPORTED_MODULE_4__.J(this.config.client.globals)),
						(this.logger =
							services?.logger ||
								...
								scriptContextProfiles.forEach((profile) => {
									...
									new _searchspring_snap_toolbox__WEBPACK_IMPORTED_MODULE_6__.b(targetsArr, async (target, elem, originalElem) => {
										if (target.profile?.profile) {
											const profileRequestGlobals = { ...requestGlobals, profile: target.profile?.options, tag: target.profile.profile },
												profileContext = deepmerge__WEBPACK_IMPORTED_MODULE_1___default()(this.context, {
													globals: scriptContextGlobals,
													profile: target.profile,
												});
											elemContext.custom && (profileContext.custom = elemContext.custom),
			...
			async function readyTheController(instance, services, injectedElem, context, profileCount, elem, controllerGlobals, target) {
				...
				if (!tag) return void instance.logger.warn("'profile' is missing from <script> tag, skipping this profile", elem);
