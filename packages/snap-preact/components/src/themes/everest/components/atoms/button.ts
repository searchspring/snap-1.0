...
const buttonStyleScript = (props: ButtonProps) => {
	...
	const bottomBorderColor = new Color(props?.backgroundColor || variables?.colors?.secondary || undefined);
	const fontColor = buttonColor.isDark() || buttonColor.hex().toLowerCase() == '#d15120' ? Color(custom.colors.white) : Color(custom.colors.black);
