...
describe('Result Component', () => {
	...
	it('renders', () => {
		const rendered = render(<VariantResult result={mockResults[0] as Product} />);
	...
	it('renders with "sale" classname when on sale', () => {
		...
		const rendered = render(<VariantResult result={product} />);
		...
		product.mappings.core!.msrp = 1.99;
		const reRendered = render(<VariantResult result={product} />);
	...
	it('renders details', () => {
		...
		const rendered = render(<VariantResult {...args} />);
	...
	it('should display a fallback image', () => {
		...
		const rendered = render(<VariantResult result={mockResults[1] as Product} />);
		const imageElement = rendered.container.querySelector('.ss__result .ss__result__image-wrapper .ss__image img');
		expect(imageElement).toHaveAttribute('src', FALLBACK_IMAGE_URL);
	});

	it('can change the layout', () => {
		const rendered = render(<VariantResult result={mockResults[1] as Product} layout={ResultsLayout.list} />);
	...
	it('can set a custom onClick function', async () => {
		...
		const rendered = render(<VariantResult result={mockResults[1] as Product} onClick={onClickFunc} />);
	...
	it('can render ratings', () => {
		const rendered = render(<VariantResult result={mockResults[1] as Product} hideRating={false} />);
		const resultElement = rendered.container.querySelector('.ss__result');
		const ratingElement = rendered.container.querySelector('.ss__result__rating');
	...
	it('can render addToCart button', async () => {
		...
		// @ts-ignore
		const rendered = render(<VariantResult controller={controller} result={mockResults[1] as Product} hideAddToCartButton={false} />);
	...
	it('can pass additional function to call on addToCart button click', () => {
		...
		const rendered = render(
			// @ts-ignore
			<VariantResult onAddToCartClick={customFunc} controller={controller} result={mockResults[1] as Product} hideAddToCartButton={false} />
		);
	...
	it('renders with classname', () => {
		...
		const rendered = render(<VariantResult result={mockResults[1] as Product} className={className} />);
	...
	it('can disable styles', () => {
		const rendered = render(<VariantResult result={mockResults[1] as Product} disableStyles />);

		const resultElement = rendered.container.querySelector('.ss__result.ss__result--variant-result');

		expect(resultElement?.classList).toHaveLength(4);
	});
	...
	it('can show variant selections with hideVariantSelections false', () => {
		...
		const rendered = render(<VariantResult result={resultWithVariants} hideVariantSelections={false} />);
	...
	it('hides variant selections by default', () => {
		...
		const rendered = render(<VariantResult result={resultWithVariants} />);
	...
	it('hides variant selections when hideVariantSelections is true', () => {
		...
		const rendered = render(<VariantResult result={resultWithVariants} hideVariantSelections={true} />);
	...
	it('immediately available lang options', async () => {
		...
		langOptions.forEach((option) => {
			langObjs.forEach((langObj) => {
				...
				console.log(lang);
				// @ts-ignore
				const rendered = render(<VariantResult result={mockResults[1] as Product} lang={lang} hideAddToCartButton={false} />);
				rendered.debug();
...
describe('Result theming works', () => {
	it('is themeable with ThemeProvider', () => {
		...
		const rendered = render(
			<ThemeProvider theme={globalTheme}>
				<VariantResult result={mockResults[0] as Product} />
			</ThemeProvider>
		);
	...
	it('is themeable with theme prop', () => {
		...
		const rendered = render(<VariantResult result={mockResults[0] as Product} theme={propTheme} />);
		const result = rendered.container.querySelector('.ss__result');
		const title = rendered.container.querySelector('.ss__result__details__title');
		expect(result).toBeInTheDocument();
		expect(title).not.toBeInTheDocument();
	...
	it('is themeable and theme prop overrides ThemeProvider', () => {
		...
		const rendered = render(
			<ThemeProvider theme={globalTheme}>
				<VariantResult result={mockResults[0] as Product} theme={propTheme} />
			</ThemeProvider>
		);
