...
### icon
The `icon` prop specifies a path within the `Icon` component paths (see Icon Gallery). Pass `False` to disable the Icon from rendering

```tsx
<Filter facetLabel={filter.facet.label} icon={'close-thin'} />
```
