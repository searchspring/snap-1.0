...
describe('Hybrid Api', () => {
	...
	it('can call getSearch', async () => {
		...
		const fetchUrl = `https://8uyt2m.a.athoscommerce.net/v1/search?siteId=8uyt2m&ajaxCatalog=snap%2Fclient%2F${version}&resultsFormat=native`;
	...
	it('can call getAutcomplete', async () => {
		...
		const cacheKey =
			'https://8uyt2m.a.athoscommerce.net/v1/suggest?siteId=8uyt2m&language=en&query=undefined&suggestionCount=5&disableSpellCorrect=true';
